...
export default function KnowledgeBase() {
  ...
  async function handleDocumentUpload(event) {
    ...
    if (!editingDocument && documentForm.file?.size > 100 * 1024 * 1024) {
      toast.error('O documento excede o limite de 100 MB.');
      return;
    }
    ...
    <div style={s.page}>
      ...
      {showDocumentModal ? <ModalShell kicker="Base documental" title={editingDocument ? 'Editar dados do documento' : 'Anexar manual ou portfólio'} onClose={() => { setShowDocumentModal(false); setEditingDocument(null); }} maxWidth="42rem">
        <form onSubmit={handleDocumentUpload} style={s.modalBody}>
          ...
          <div style={s.formGrid}><label style={s.fieldGroup}><span style={s.label}>Idioma do documento</span><select style={s.input} value={documentForm.language} onChange={(e) => setDocumentForm({ ...documentForm, language: e.target.value })}><option value="pt-BR">Português</option><option value="en">Inglês</option><option value="es">Espanhol</option></select><small style={s.fieldHelp}>Manuais em inglês: a pergunta do técnico (em português) é traduzida automaticamente na busca.</small></label><label style={s.fieldGroup}><span style={s.label}>Versão</span><input style={s.input} value={documentForm.version} onChange={(e) => setDocumentForm({ ...documentForm, version: e.target.value })} /></label></div>
          {!editingDocument ? <label style={s.fieldGroup}><span style={s.label}>Arquivo (máx. 100 MB)</span><input style={s.input} type="file" required accept=".pdf,.docx,.txt,.jpg,.jpeg,.png,.webp" onChange={(e) => setDocumentForm({ ...documentForm, file: e.target.files?.[0] || null })} /></label> : null}
          {!editingDocument ? <><label style={s.fieldGroup}><span style={s.label}>Substitui uma versão anterior? (opcional)</span><select style={s.input} value={documentForm.supersedesId} onChange={(e) => setDocumentForm({ ...documentForm, supersedesId: e.target.value })}><option value="">Não, é um documento novo</option>{documents.filter((item) => item.status !== 'REPLACED').map((item) => <option key={item.id} value={item.id}>{item.title}{item.version ? ` — versão ${item.version}` : ''}</option>)}</select><small style={s.fieldHelp}>Ao publicar a nova versão, a anterior será retirada automaticamente das respostas.</small></label><div style={s.noMatch}><AlertTriangle size={17} /> O processamento ocorre em fila isolada, um documento por vez. O arquivo original fica preservado mesmo se a indexação for interrompida. Ele só orienta o bot após a publicação.</div></> : <div style={s.noMatch}><AlertTriangle size={17} /> A correção dos dados é aplicada imediatamente e não altera o arquivo nem exige nova indexação.</div>}
